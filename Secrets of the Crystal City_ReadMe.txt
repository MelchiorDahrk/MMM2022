...
1.7 - Added crime system to Uxith-Kei faction (talk to Ravos to clear the bounty if you catch too much heat), additional underwater detailing in `Massama, Commons` and `Arkng-Zel, Ring of Yendorr` cells, additional detailing in `Massama, Commons` cell, additional detailing in the `Massama, City` mining area, more secrets to discover throughout the mod, improved performance in `Massama, City` and `Massama, Commons` cells, more fitting waterfall for the aqueduct leading into the main city, cleaned dialogue, bug fixes